...
public class RemoteFeedbackReceiver

{
	public class WorkArriveListener implements MessageListener {
		public void onMessage(Message msg) {
			...
			try {
				System.out.println("receive a remote feedback");
	...
	private static RemoteFeedbackReceiver remoteWorkReceiver = new RemoteFeedbackReceiver();

	public static RemoteFeedbackReceiver getRemoteWorkReceiver() {
		return remoteWorkReceiver;
	}

	private RemoteFeedbackReceiver() {
		init();
	}
	...
	private void init() {
		try {
			...
			queue = (Queue) ctx
					.lookup(RemoteWorkManagerConstant.REMOTE_WORK_FEEDBACK_QUEUE);
