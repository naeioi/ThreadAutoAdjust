...
import com.onceas.runtime.work.ThreadPoolRuntime;
...
import com.onceas.work.IncrementAdvisor;
...
public final class RequestManager {
	...
	private RequestManager() {
		...
		incrementAdvisor = new IncrementAdvisor();
		(new Timer(true)).schedule(incrementAdvisor,
				INCREMENT_ADVISOR_START_DELAY, INCREMENT_ADVISOR_PERIOD);
		try {
			ThreadPoolRuntimeMBean threadpoolruntimembean = ThreadPoolRuntime
					.createThreadPoolRuntimeMBean(this);
			MBeanServer server = com.onceas.util.jmx.MBeanServerLoader
					.loadOnceas();
			server.registerMBean(threadpoolruntimembean, threadpoolruntimembean
					.getObjectName());
			HealthMonitorMService.register("threadpool",
					threadpoolruntimembean, false);
	...
	public static void initInternalRequests() {
		ACTIVATE_REQUEST
				.setWorkManager((ServerWorkManagerImpl) WorkManagerFactory
	...
	/**
	 * Enqueue this workadapter On Reaching MaxConstraint
	 * 
	 * @param workadapter
	 * @return
	 */
	private boolean enqueueOnReachingMaxConstraint(WorkAdapter workadapter) {
		...
		if (maxthreadsconstraint != null
				&& maxthreadsconstraint.isConstraintReached()) {
			WorkAdapter workadapter1 = workadapter;
			if (minthreadsconstraint != null) {
				// get the minthreadsconstraint work proxy which will get work
				// from queue by getNext()
				workadapter1 = minthreadsconstraint.add(workadapter);
				// the following comment made by syk cause:updating max.count
				// when max.coutn==min.count cannot
				// immediatelly get work from maxqueue because of that queue is
				// empty.
				/*
				 * this is a special case: placing workadapters only in minqueue
				 * cause:when max.count=min.count,thread only can get work from
				 * minqueue, never access maxqueue or priority queue(see
				 * getMinConstraintWork() used in getNext()),so if this continue
				 * for a long time,maxqueue and priority queue will be full of
				 * min.proxys,that is terrible! so at the same time(commenting
				 * statements) i change getMinConstraintWork() to ensure
				 * gainning a work from maxqueue or priority instead of minqueue
				 * when min.count == max.count
				 * 
				 */
				// if (minthreadsconstraint.getCount() == maxthreadsconstraint
				// .getCount()) {
				// queueDepth++;
				// return true;
				// }
			...
			if (idleThreads.size() > 0) {
				// not busy so add to maxconstraint queue
				maxthreadsconstraint.add(workadapter1);
			} else {
				queue.add(workadapter1, workadapter.requestClass);
				queueNonEmpty++;
			}
			queueDepth++;
			return true;
	...
	public boolean executeIt(WorkAdapter workadapter) {
		...
		synchronized (this) {
			...
			MinThreadsConstraint minthreadsconstraint = workadapter
					.getMinThreadsConstraint();
			if (executethread == null && minthreadsconstraint != null
					&& !minthreadsconstraint.isConstraintSatisfied()) {
				if (standbyThreadPool.size() == 0) {
					flag = true;
					i = threadID();
				} else {
					// if minthreadsconstraint is not Satisfied then allocate a
					// thread from standbyPool
					// why donot use getFromStandbyPool()() method ? the
					// following code does not
					// set standBy state to false so comment it :by syk
					// executethread = (ExecuteThread) standbyThreadPool.pop();
					executethread = getFromStandbyPool();
				}
				mtcDepartures++;
			}
			if (executethread != null || flag) {
		...
		departures++;
		if (flag) {
			// if minthreadsconstraint is not Satisfied and
			// standbyThreadPool.size() == 0
			// then creat a thread
	...
	private void addToPriorityQueue(WorkAdapter workadapter) {
		...
		WorkAdapter workadapter1 = minthreadsconstraint != null ? minthreadsconstraint
				.add(workadapter)
				: workadapter;
		queue.add(workadapter1, workadapter.requestClass);
		queueDepth++;
	 ...
	 * @return
	 */
	private WorkAdapter getNext(WorkAdapter workadapter, long l) {
		// first check if the last work has min or max contraint
		// if has, then excute
		// 如果此work关联的wm含有min，且运行的线程数小于min.count则从min的queue中获得一个workadapter
		// 假定min<=max：by syk
		WorkAdapter workadapter1 = getMinConstraintWork(workadapter, l);
		if (workadapter1 != null)
			return workadapter1;
		// 如果此work关联的wm含有max，则从max的获得一个workadapter的子类max.ProxyEntry（null if size
		...
		// readyToRun()对workadapter检查是否达到关联的max.count限制；对min.proxyEntry检查minqueue是否为空，但没有检查max.coutn限制？？
		// by syk
		for (; workadapter2 != null && !workadapter2.readyToRun(); workadapter2 = getFromPriorityQueue())
			;
		...
		// workadapter2 may be min.proxy by syk
		return workadapter2.getEffective();
	}
	...
	private WorkAdapter getFromPriorityQueue() {
		...
		while ((workadapter = (WorkAdapter) queue.pop()) != null) {
			...
			// 类似 max.getNext()从maxqueue中取任务，需要检查 priority
			// queue中的workadapter.started ???? by syk
			if (workadapter1 != null)
				return workadapter1;
		}
		return null;
	...
	private void fireQueueEmptied(long l, long l1) {
		synchronized (requestClasses) {
			...
			for (Iterator iterator = requestClasses.iterator(); iterator
					.hasNext(); requestclass.queueEmptied(l, f))
				requestclass = (RequestClass) iterator.next();
	...
	public synchronized ArrayList getStuckThreads(long l) {
		...
		if (hogs.size() == 0 || l == 0L)
			return null;
		Iterator iterator = hogs.iterator();// 这里的hogs存储的是可疑的线程
		do {
			if (!iterator.hasNext())
				break;
