...
public class ReleaseWorkReceiver

{
	public class WorkArriveListener implements MessageListener {
		public void onMessage(Message msg) {
			...
			try {
				System.out.println("receive a release work");
	...
	private static ReleaseWorkReceiver releaseWorkReceiver = new ReleaseWorkReceiver();

	public static ReleaseWorkReceiver getRemoteWorkReceiver() {
		return releaseWorkReceiver;
	}

	private ReleaseWorkReceiver() {
		init();
	}
	...
	private void init() {
		try {
			...
			queue = (Queue) ctx
					.lookup(RemoteWorkManagerConstant.REMOTE_WORK_RELEASE_QUEUE);
