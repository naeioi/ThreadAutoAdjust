 ...
 * wm 部署时切入的工具类
 * 基本用法：
 * 		1）在部署web应用的方法最后执行doAfterDeploy（url,appName）;
 * 		
 * 	  	2）在解部署web应用的方法最后执行doAfterUnDeploy（url,appName）;
 * 		
 * @author yk
 *
 */
public class DeployPointCut {

	public static void doAfterDeploy(URL url, String appName){
		/*
		 * check whether the VM server has started
		 */
		if( !WMServer.isStarted()){
			synchronized (DeployPointCut.class) {
				if(!WMServer.isStarted()){
...
//			appName = stripStartSlash(appName);
	...
	private static WmDD parse(URL url){
			File wmDDFile= null;
	...
	public static void doAfterUndeploy(String appName){
		//stripping starting slash of appName
		appName = stripStartSlash(appName);
	...
	private static  String stripStartSlash(String s){
